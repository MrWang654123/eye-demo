...
public class XYAxis extends Axis {
	...
	private int mTickMarksMargin = 10;
	...
	protected void renderVerticalTick(Canvas canvas, 
		...
		case BOTTOM:
			...
			if(this.isShowAxisLabels())
			{
				labelsStartY = 20 + marksStopY + getTickLabelMargin()
		...
		if (isShowAxisLabels()) {
			...
			if(text.contains("qs_nor")){
				text = text.replace("qs_nor","");
				if(!TextUtils.isEmpty(text)){
					DrawHelper.getInstance().drawLabelCircle(getFormatterLabel(text),labelX, currY,canvas,getTickLabelPaint(),false);
				}
			}else if(text.contains("qs_sel")){
				text = text.replace("qs_sel","");
				if(!TextUtils.isEmpty(text)){
					DrawHelper.getInstance().drawLabelCircle(getFormatterLabel(text),labelX, currY,canvas,getTickLabelPaint(),true);
				}
			}else {
				//定制化显示格式
											...
											float centerX, float centerY, String text,float width)
	{
		...
		{
			//标签绘制在一行中
     	   DrawHelper.getInstance().drawRotateText(label,centerX-30, centerY,
								getTickLabelRotateAngle(), canvas,getTickMarksPaint());
		}else{	//Multi line			
	...
	public int getmTickMarksMargin() {
		return mTickMarksMargin;
	}

	public void setmTickMarksMargin(int mTickMarksMargin) {
		this.mTickMarksMargin = mTickMarksMargin;
	}
