...
public class DrawHelper {
	 ...
	 * @param isSelect
	 */
	public void drawLabelCircle(String text,float x ,float y, Canvas canvas, Paint paint,boolean isSelect){
		Paint p = new Paint(Paint.ANTI_ALIAS_FLAG);
		if(isSelect){
			p.setColor(Color.parseColor("#ffa200"));
		}else{
			p.setColor(Color.parseColor("#666666"));
		}
		canvas.drawCircle(x+5,y-5,20,p);

		p.setColor(Color.WHITE);
		p.setTextSize(20);
		drawText(canvas,p,text,x,y);
	}
}
