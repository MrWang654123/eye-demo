...
public class ViewPagerIndicate extends HorizontalScrollView implements OnClickListener {
    ...
    public void resetText(int tabId, String[] titles, int[] colors) {
        ...
        for (int i = 0; i < titles.length; i++) {
            View view = mInflater.inflate(tabId, null);
            TextView tv = (TextView) view.findViewById(R.id.tv_page_title);
            tv.setText(titles[i]);
            tv.setTag(i);
    ...
    private void setTextHighlight(int pos) {
        for (int i = 0; i < mTextViews.size(); i++) {
            ...
            if (i == pos){
                tv.setTextColor(mTextHighlightColor);
                tv.setBackgroundResource(R.drawable.shape_report_indicate_select);
            } else{
                tv.setTextColor(mTextNormalColor);
                tv.setBackgroundResource(R.drawable.shape_report_indicate_nor);
            }
        }
    }
